...
## โหมดแข่งขัน (เคล็ดลัด)

* ถ้าเวลาน้อยมาก ให้ใช้ **3 ไฟล์**: `config.php`, `functions.php`, `index.php`
* ไม่ใช้ Bootstrap ก็ได้ (มี CSS เล็ก ๆ ใน `index.php`)
* พิมพ์ซ้ำได้ง่าย อ่านโครงสร้างชัดเจน

---

## ความปลอดภัย (ข้อควรทราบ)

* รุ่นนี้ตั้งใจ “ลดความซับซ้อน” เพื่อการสอนและแข่ง:

  * ไม่มี CSRF token, ไม่มี validation ขั้นสูง, ไม่มี rate limit
  * อย่าใช้ในงาน production โดยไม่เพิ่มการป้องกัน
* ใช้ `password_hash()`/`password_verify()` แล้ว แต่ยังควรเพิ่มนโยบายรหัสผ่าน/ล็อกเอาต์/ยืนยันตัวตน หากไปต่อยอดจริง

---

## การต่อยอด (หลังการแข่งขัน)

* เพิ่มหน้าโปรไฟล์/เปลี่ยนรหัส/ลืมรหัสผ่าน
* เพิ่มกราฟ/สรุปผล/Export CSV
* เพิ่ม Leaderboard รายแผนก/รายห้อง/รายบุคคล
* เพิ่มระบบค้นหา/กรอง/หน้ารายงาน
* เปิดใช้ Bootstrap ออฟไลน์เพื่อ UI ที่สวยขึ้น

---

## Troubleshooting

* “ล็อกอินไม่ได้” → ตรวจ hash ของ admin, ตรวจชื่อ DB/ผู้ใช้/รหัสผ่านใน `config.php`
* “หน้าไม่ขึ้นข้อมูลกิจกรรม” → เช็กตาราง `activity_types` มีข้อมูล/ `is_active=1`
* “Foreign key error” → ตรวจลำดับการ insert: ต้องมี `users`/`activity_types` ก่อนค่อย insert `logs`

---

## ใบอนุญาต

MIT License — ใช้เพื่อการศึกษา/ฝึกซ้อม/ต่อยอดได้อย่างอิสระ

---

## เครดิต

พัฒนาเพื่อการเรียนรู้เชิงปฏิบัติและการเตรียมตัวแข่งขันทักษะอาชีพด้านเทคโนโลยีธุรกิจดิจิทัล.
